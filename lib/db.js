...
export async function setCalendarMarker(user, dateString, markerColor) {
  if (!user || !dateString) return;
  try {
    const activityRef = doc(db, 'users', user.uid, 'dailyActivity', dateString);
    if (!markerColor || markerColor === 'clear') {
      // Allow clearing a marker
      await setDoc(activityRef, { color: null, date: dateString, lastUpdated: serverTimestamp() }, { merge: true });
    } else {
